...
const Step4Confirmation: React.FC<Step4ConfirmationProps> = ({ values, navigate, isClientRegistration }) => {
    return (
        <div className="text-center animate-fade-in py-8">
            <CheckCircleIcon className="w-20 h-20 text-green-500 mx-auto mb-4" />
            <h2 className="text-3xl font-serif font-bold text-brand-dark mb-4">
                ¡Felicitaciones, {isClientRegistration ? values.name : values.companyName}!
            </h2>
            <p className="text-brand-dark opacity-80 max-w-lg mx-auto mb-8">
                {isClientRegistration ? 
                    'Tu cuenta ha sido creada exitosamente. ¡Bienvenido a Mi Boda Ideal!' :
