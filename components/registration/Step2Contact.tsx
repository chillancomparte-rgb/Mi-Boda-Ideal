...
const Step2Contact: React.FC<Step2ContactProps> = ({ nextStep, prevStep, handleChange, values, isClientRegistration }) => {
    const continueStep = (e: React.FormEvent) => {
        e.preventDefault();
        let isValid = false;
        if (isClientRegistration) {
            isValid = values.phone && values.location;
        } else {
            isValid = values.phone && values.city;
        }

        if (isValid) {
            nextStep();
        } else {
            alert('Por favor, completa el teléfono y la ubicación.');
        }
    };
    ...
    return (
        <form onSubmit={continueStep} className="animate-fade-in">
            <h2 className="text-2xl font-serif font-bold text-brand-dark mb-6">
                {isClientRegistration ? '2. Tu Información de Contacto' : '2. Información de Contacto y Ubicación'}
            </h2>
